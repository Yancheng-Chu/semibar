...
const SemiEllipseMenu: React.FC<SemiEllipseMenuProps> = ({ options, onOptionClick }) => {
  ...
  const handleOptionClick = (option: MenuOption, index: number) => {
    ...
    if (contentRef.current) {
      ...
      // 判断点击的是哪一组（上、中、下）
      const groupIndex = Math.floor(index / options.length); // 0=上组, 1=中组, 2=下组
      
      let targetScrollTop;
      if (groupIndex === 0) {
        // 点击上组：滚动到上组对应位置（往上滚）
        targetScrollTop = optionIndex * itemHeight - centerOffset;
      } else if (groupIndex === 2) {
        // 点击下组：滚动到下组对应位置（往下滚）
        targetScrollTop = 2 * options.length * itemHeight + optionIndex * itemHeight - centerOffset;
      } else {
        // 点击中组：滚动到中组对应位置
        targetScrollTop = options.length * itemHeight + optionIndex * itemHeight - centerOffset;
      }
